...
在分析的第一阶段，输入是字符串形式的源代码，经过词法分析器（Lexical Analyzer）的扫描（Scan）后，输出为词法单元或记号（Token）的序列。简而言之，第一阶段将字符串流被分割成一个个token。Token主要包括两部分，一部分是分割出的内容，也叫词位（Lexeme），一部分是该内容的类别，因此一个Token可以表示为：(Lexeme, Type)。
